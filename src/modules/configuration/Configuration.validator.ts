...
export const ConfigurationSchema = {
  ...
  "properties": {
    ...
    "ignoreTokenizedStocks": {
      "description": "If true, it will not try to buy tokenized stocks. E.g. prohibited in your country",
      "type": "boolean"
    },
    "mainMarket": {
      "$ref": "#/definitions/MainMarket",
      "description": "Market to trade all coins on. E.g. BTC or USDT\nOnly tested USDT so far"
    },
    "minNegativeTicks": {
      "description": "Will reject when negative ticks are equal or below this number",
      "type": "number"
    },
    "refreshTimeout": {
...
export const isConfiguration = ajv.compile(ConfigurationSchema) as ValidateFunction<Configuration>;
export default function validate(value: unknown): Configuration {
  if (isConfiguration(value)) {
    return value;
  } else {
    throw new Error(
      ajv.errorsText(isConfiguration.errors!.filter((e: any) => e.keyword !== 'if'), {dataVar: 'Configuration'}) +
      '\n\n' +
      inspect(value),
    );
  }
